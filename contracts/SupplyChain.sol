  ...
  */
   ...
   */
    ...
    modifier forSale(uint256 _sku) {
        ...
        require(_seller != address(0), "should have a price for sale");
        require(_state == State.ForSale, "should have ForSale state");
        _;
    }

    modifier sold(uint256 _sku) {
        State _state = items[_sku].state;
        require(_state == State.Sold, "should have Sold state");
        _;
    }

    modifier shipped(uint256 _sku) {
        State _state = items[_sku].state;
        require(_state == State.Shipped, "should have Shipped state");
        _;
    }

    modifier received(uint256 _sku) {
        State _state = items[_sku].state;
        require(_state == State.Received, "should have Received state");
        _;
    }
